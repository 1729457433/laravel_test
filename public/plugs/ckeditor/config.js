...
CKEDITOR.editorConfig = function (config) {
    ...
    config.toolbar = [
        ...
        {name: 'basicstyles', items: ['Bold', 'RemoveFormat', 'CopyFormatting', 'TextColor']},//'Italic', , 'BGColor', 'Underline', 'Strike'
        {name: 'align', items: ['JustifyLeft', 'JustifyCenter', 'JustifyRight', 'JustifyBlock']},
        // {name: 'paragraph', items: ['NumberedList', 'BulletedList', 'Outdent', 'Indent', 'Blockquote', 'Link', 'Unlink']},
        {name: 'uimage', items: ['UploadImage']},//'Table',
    ...
    init: function (editor) {
        ...
        editor.addCommand('uimage', {
            exec: function (editor) {
                ...
                var url = '/index.php/admin/plugs/upfile.html?mode=one&type=png,jpg,gif,jpeg&field=' + field;
                $('<input type="hidden">').attr('name', field).appendTo(editor.element.$).on('change', function () {
                    var element = CKEDITOR.dom.element.createFromHtml('<img src="' + this.value + '" style="max-width:500px;width: 100%;" border="0" title="image" />');
